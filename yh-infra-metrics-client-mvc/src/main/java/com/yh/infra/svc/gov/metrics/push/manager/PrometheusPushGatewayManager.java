...
package com.yh.infra.svc.gov.metrics.push.manager;
 ...
 * {@link PushGateway Prometheus PushGateway}. Handles the scheduling of push
 * operations, error handling and shutdown operations.
 *
...
public class PrometheusPushGatewayManager {
	...
	/**
	 * Create a new {@link PrometheusPushGatewayManager} instance using a single
	 * threaded {@link TaskScheduler}.
	 * 
	 ...
	 *                          context is closed.
	 */
	public PrometheusPushGatewayManager(PushGateway pushGateway, CollectorRegistry registry, int pushRate, String job, Map<String, String> groupingKeys,
	 ...
	 *                          context is closed.
	 */
	public PrometheusPushGatewayManager(PushGateway pushGateway, CollectorRegistry registry, TaskScheduler scheduler, int pushRate, String job,
		...
		this.scheduled = this.scheduler.scheduleAtFixedRate(new Runnable() {
			@Override
			public void run() {
				PrometheusPushGatewayManager.this.push();
			}
		}, pushRate);
	}
	...
	private void push() {
		try {
			...
			String message = "Unable to locate prometheus push gateway host" + (StringUtils.hasLength(host) ? " '" + host + "'" : "")
	...
	public enum ShutdownOperation {
		...
		public static ShutdownOperation getShutdownOperation(String operation) {
			switch (operation) {
			case "push":
				return ShutdownOperation.PUSH;
			case "delete":
				return ShutdownOperation.DELETE;
			default:
				return ShutdownOperation.NONE;
			}
		}
	}
	...
	static class PushGatewayTaskScheduler extends ThreadPoolTaskScheduler {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -588787395623611552L;
