...
public class SendReceiveService {
	...
	Map<String, String> retMap = uacService.sendRequestWithToken(url, msgBody,
	...
	if ((tmp != null) || (!SdkCommonConstant.HTTP_STATUS_OK.equals(status))) {
	    logger.warn("Get error when post JSON request. app:{}, error:{}, status:{}, result:{}", req.getAppId(), tmp,
		    status, result);
	    return null;
	}
