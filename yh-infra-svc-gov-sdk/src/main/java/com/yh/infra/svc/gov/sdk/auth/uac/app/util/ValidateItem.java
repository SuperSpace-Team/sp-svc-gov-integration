...
public class ValidateItem {
	...
	public static final String EMAIL_REG="^\\s*\\w+(?:\\.{0,1}[\\w-]+)*@[a-zA-Z0-9]+(?:[-.][a-zA-Z0-9]+)*\\.[a-zA-Z]+\\s*$";
	...
	private boolean validEmail(Object value){
		...
		if(dataType== DataType.string){
			String str=(String)value;
			if(StringUtils.isNotBlank(str)&&!str.matches(EMAIL_REG))
				return false;
		}
	...
	private boolean validMobile(Object value){
		...
		if(dataType== DataType.string){
			String str=(String)value;
			if(StringUtils.isNotBlank(str)&&!str.matches(MOBILE_REG))
				return false;
		}
